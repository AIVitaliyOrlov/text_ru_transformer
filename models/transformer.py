...
from models.layers import EncoderLayer, DecoderLayer, EncoderPreNormLayer, DecoderPreNormLayer, \
    DecoderPreNormLayerWithoutEncoder, DecoderLayerWithoutEncoder
...
class DecoderWithoutEncoderInput(nn.Module):
    """A class for implementing Transformer Decoder without encoder input."""

    def __init__(self, config):
        """Initializes the Encoder consisting of multiple EncoderLayer layers."""
        super(DecoderWithoutEncoderInput, self).__init__()
        decoder_layer = DecoderPreNormLayerWithoutEncoder if config.pre_normalization else DecoderLayerWithoutEncoder
        self.layers = torch.nn.ModuleList([decoder_layer(config) for _ in range(config.layers_num)])
        self.config = config
    ...
    def forward(self, inputs: torch.Tensor,
        ...
        for layer in self.layers:
            outputs, _ = layer(outputs, tgt_mask)
            #outputs, self_attention_weights = layer(outputs, tgt_mask)
            #all_self_attention_weights.append(self_attention_weights)

        return outputs#, all_self_attention_weights
...
class TextDecoderTransformer(nn.Module):
    ...
    def __init__(self, config, vocabulary_size):
        ...
        self.embeddings = Embedding(vocabulary_size, config.model.d_model)

        self.positional_encoding = PositionalEncoding(
            config.model.max_sequence_length, config.model.d_model, config.model.dropout_rate
        )
        self.decoder = DecoderWithoutEncoderInput(config.model)
        self.output = TransformerOutput(config.model.d_model, vocabulary_size)
    ...
    def forward(self, inputs: torch.Tensor,
        ...
        inputs_embeddings = self.positional_encoding(self.embeddings(inputs))
        #decoder_output, decoder_self_attention_weights = self.decoder(inputs_embeddings, tgt_mask)
        decoder_output = self.decoder(inputs_embeddings, tgt_mask)
